  ...
  alter table if exists weekly_schedule_settings
    add column if not exists category text not null default 'regular';

  alter table if exists weekly_schedule_settings
    drop constraint if exists weekly_schedule_settings_day_of_week_key;

  drop index if exists weekly_schedule_settings_day_of_week_key;

  create unique index if not exists weekly_schedule_settings_category_day_uq on weekly_schedule_settings (category, day_of_week);
  ...
  drop trigger if exists weekly_schedule_settings_set_updated_at on weekly_schedule_settings;
  create trigger weekly_schedule_settings_set_updated_at
  before update on weekly_schedule_settings
  for each row
